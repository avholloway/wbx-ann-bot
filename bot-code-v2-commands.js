...
export default defineComponent({
  async run({ steps, $ }) {
    ...
    if (/\/help/.test(source_message)) {
      log('COMMAND: /help');
      const commands = [
        {cmd: '/help', desc: 'Displays this help'},
        {cmd: '/rooms', desc: 'Displays a list of rooms I am in (not including this room)'},
        {cmd: '/spaces', desc: 'Alias for \`/rooms\`'},
        {cmd: '/reach', desc: 'Displays a count of all spaces and all people who I can reach'}
      ]
      const list_of_commands = commands.map(command => `* \`${command.cmd}\` - ${command.desc}`).join('\n');
      await send_message(admin_room, `🤖 These are the commands I know:\n${list_of_commands}`, message_id);
      return $.flow.exit('/help command complete');
    }

    if (/\/(rooms|spaces)/.test(source_message)) {
      log('COMMAND: /rooms');
      const list_of_rooms = rooms.map(room => `* ${room.title}`).join('\n');
      await send_message(admin_room, `🚪 These are all of the rooms I am in:\n${list_of_rooms}`, message_id);
    ...
    if (/\/reach/.test(source_message)) {
      ...
      for (const room of rooms) {
        log({room});
        const { items } = await got_client.get(`memberships?roomId=${room.id}`).json();
        log({items});
        const humans = items.filter(item => !/@webex\.bot/.test(item.personEmail));
        people.push(...humans);
      }
      log({people});
      people = people.filter((value, index, self) =>
        index === self.findIndex(human => (
          human.personEmail === value.personEmail
        ))
      );
      log({people});
      await send_message(admin_room, `📈 I have a reach of **${people.length} people**, across **${rooms.length} rooms**.`, message_id);
